...
- `.env` (OpenAI / image-host / Threads creds, `POST_CRON`)
...
4. `upload_image.upload(...)` — host the image (catbox by default), get public URL.
...
## Manual run / test
```bash
python tools/run_bot.py --post-now              # full: OpenAI + image host + Threads
